...
Note #2: Very rarely, the program may generate an incomplete solution. This is because the program is designed using the A* (Alpha-Star) algorithm, which is essentially a trial-and-error method of searching (huge oversimplification by the way). By default, the program terminates once it has checked 5000 board states. If you ever run into this problem, I recommend inputting the following command into command prompt: "python THM.py yourname 100 10000" (without quotes and replace yourname). That command will allow the program to check 10000 boards instead of 5000, increasing the likelihood of finding a more complete solution (You can change 10000 to any number you'd like by the way; 7500 would also be sufficient. You could also decrease the number if you want to speed up the runtime, although this makes the returning of an incomplete and suboptimal solution more likely).
